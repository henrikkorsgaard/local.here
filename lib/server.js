...
( function () {
    ...
    function server() {
        ...
        function listen( config ) {
            ...
            broadcast( {
                type: "change",
                msg: "Server is up and listening on port: " + config.port,
                origin: __filename
            } );
            ...
            ws.on( 'connection', function ( con ) {
                ...
                con.on( 'message', function ( msg ) {
                    ...
                    try {
                        ...
                        sendMessage({"Error":"Unable to parse request. The service only accept a valid stringified JSON object."} );
                    }
                    ...
                    if ( piConnecting && data && data.hasOwnProperty( 'request' ) && data.hasOwnProperty( 'signature' )) {
						...
						device.isDeviceOnLocalAreaNetwork(ip, user_agent, function(bool){
							if(bool){
								...
								sendMessage({"Error":"Invalid request. You need to on the network for this to work (try reconnecting to "+config.ssid+")"} );
							}
							
						});
                    } else {
                    	sendMessage({"Error":"Invalid request. Essential information might be missing!"} );
                    }
                } );
