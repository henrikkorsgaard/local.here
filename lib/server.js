                ...
                con.on( 'message', function ( data ) {
					if(piConnecting){
	                    ...
	                    if ( request && request.hasOwnProperty( 'request' ) && api.hasOwnProperty( request.request ) ) {
							getDevice(ip, function(d){
								if(d){
									if(api[request.request].permissions.indexOf('local') > -1){
										api[request.request].function(request, con, d);
									}
								} else {
									con.close( 1003, "Unable to handle request. IP/device not proximate/local to PI." );
								}
							});
	                    } else {
				...
				function getDevice(ip, callback){
                    device.getDevice( ip, function ( err, result ) {
                        if ( err ) {
                            ...
                            activeDevice = result;
                            device.upsertDevice( result, function ( err ) {
                                if ( err ) {
                                    console.error( err );
                                }
                            } );
