            ...
            ws.on( 'connection', function ( con ) {
                ...
                con.on( 'message', function ( data ) {
                    ...
                    if ( request && request.hasOwnProperty( 'request' ) && api.hasOwnProperty( request.request ) ) {
                        getRequestPermissions(ip, request.token, function(err, permissions){
                ...
                function getRequestPermissions( ip, tokenKey, callback ) {
                    let counter = 0;
                    let permissions = ['all'];
                    device.getDevice( "192.168.1.101", function ( err, result ) {
                        if ( err ) {
                            console.error( err ); 
                        }
                        if ( result ) {
                            permissions.push( 'local' );
                            counter += 1;
                    ...
                    if ( tokenKey ) {
                        token.validateToken( tokenKey, function( err, result ) {
                            if ( err ) {
                                console.error( err );
                            }
